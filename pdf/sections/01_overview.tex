...
containing video demonstrations, task instructions, stage descriptions, and optionally manual text when available.
...
offline RL, policy selection, or trajectory evaluation. The main training recipe is now
fully specified: the reward model is supervised primarily with simulator-derived stage progress labels,
augmented by pairwise ordering labels from successful and failed trajectories, and evaluated downstream with offline RL using IQL.
The immediate implementation milestone is intentionally narrower:
the first prototype is a one-task simulation replay demo for retrieval-conditioned progress estimation,
used to validate the memory bank, retrieval quality, and progress supervision path before full reward-head training.
Downstream IQL remains the main research target, but it is not treated as the first prototype deliverable.
...
\begin{itemize}
  ...
  \item Deliver a short-horizon prototype that demonstrates retrieval-conditioned progress estimation before the full reward model is trained.
  \item Define a concrete supervision strategy for reward labels using simulator or task-derived pseudo annotations.
...
\begin{enumerate}
  \item Does context retrieved from video demonstrations, task instructions, and stage descriptions help the reward model estimate task progress more accurately?
