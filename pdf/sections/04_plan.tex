...
\begin{longtable}{p{2.1cm}p{4.0cm}p{7.6cm}}
  ...
  \midrule
  P0 & Build a one-task simulation demo & A retrieval-conditioned progress-estimation prototype with a mini memory bank, top-$k$ retrieval, and visual artifacts for rapid reporting \\
...
\section{Immediate Demo Deliverable}

The near-term deliverable is a compact prototype used to demonstrate execution readiness before full reward-model training.
The preferred execution path is a short `LIBERO` rollout when the benchmark stack can be initialized quickly under the project environment,
with a replay-based fallback kept only as a reliability guardrail for rapid reporting.
It consists of:
\begin{itemize}
  \item a demo image showing the query frame, retrieved exemplars, stage labels, and predicted progress;
  \item a progress plot comparing no-context, random-context, and retrieved-context estimates against oracle temporal progress;
  \item a short replay GIF or video with predicted progress and retrieved stage information overlaid.
\end{itemize}
