...
\begin{longtable}{p{2.1cm}p{4.0cm}p{7.6cm}}
  ...
  \midrule
  P1 & Lock benchmark and task suite & Choose `LIBERO` as the primary benchmark, define OOD splits, and finalize baselines \\
  P2 & Build the memory bank & Retrieval dataset with text, stages, clips/sub-trajectories, and a vector index \\
  P3 & Train the reward model & Retrieval-conditioned reward head or ranking model \\
  P4 & Integrate with policy learning & Offline RL or policy-selection results with success rate and sample efficiency \\
  P5 & Analyze and write up & Ablations, retrieval failure analysis, compute cost, limitations, and future work \\
  \bottomrule
\end{longtable}
